...
mooltipass.device.connect = function () {
  ...
  const mp = devices.find((device) => device.product === 'Mooltipass')
  console.log('Found', devices.length, 'devices.')
  console.log('Mooltipass found, vendor', mp.vendorId, ', product', mp.productId)

  device = new HID.HID(mp.vendorId, mp.productId)
...
mooltipass.device._sendMsg = function (queuedItem) {
  ...
  device.write(mooltipass.device.connectionId, 0, queuedItem.packet, mooltipass.device.onSendMsg)
